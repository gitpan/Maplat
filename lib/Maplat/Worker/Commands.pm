...
sub work {
	...
	foreach my $command (@commands) {
		...
		if($self->{log_all}) {
			$reph->log("OTHER", "DEBUG Command " . $command->{command} . " $printarglist started");
		}
		$dbh->commit;
		...
		if(defined($self->{extcommand}->{$command->{command}})) {
			...
			$reph->log($logtype, "Command " . $command->{command} . " not implemented"); # "Main screen turn on!"
		}
		...
		if(!$done) {
			$reph->log($logtype, "Command " . $command->{command} . " $printarglist failed");
		} elsif($self->{log_all}) {
			$reph->log("OTHER", "DEBUG Command " . $command->{command} . " $printarglist done");
		}
		$dbh->commit;
		$workCount++;
	}
